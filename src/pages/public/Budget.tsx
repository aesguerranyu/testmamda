    ...
    <div className="container mx-auto max-w-7xl px-4 sm:px-5 lg:px-6 py-5">
      ...
      <div className="mb-5">
        ...
        <p className="text-base max-w-3xl" style={{ color: "#374151" }}>
          Mayor Mamdani's first budget: $127 billion all-funds, $94 billion in agency spending. 96% of new dollars went
          to plugging inherited gaps.
        </p>
      </div>
          ...
          {[
            {
              label: "Total Budget",
              num: "$127B",
              note: "All funds FY2027 · up from $124.6B in FY2026",
              color: "#0C2788",
            },
            {
              label: "Agency Spending",
              num: "$94B",
              note: "Operating agencies · $33B to pensions & debt service",
              color: "#00933C",
            },
            {
              label: "New Investment",
              num: "$576M",
              note: "Only 4% of $14B in new spending was truly new",
              color: "#FF6319",
            },
            {
              label: "FY2028 Gap",
              num: "$6.7B",
              note: "Grows to $7.1B by FY2030 · structural deficit",
              color: "#EE352E",
            },
          ].map((kpi, i) => (
            <div key={i}>
              <p className="text-sm font-bold uppercase tracking-wide mb-1" style={{ color: "#6B7280" }}>
                {kpi.label}
              </p>
              <p className="text-5xl md:text-6xl font-bold leading-none mb-2" style={{ color: kpi.color }}>
                {kpi.num}
              </p>
              ...
              <div key={a.name} className="flex items-center gap-2.5">
                ...
                <div className="flex-[2] relative">
                  <div className="h-4 bg-gray-100 border border-gray-300 overflow-hidden">
                    <div
                      className="h-full"
                      style={{ width: `${(a.value / maxAgency) * 100}%`, backgroundColor: a.color }}
                    />
                  </div>
                </div>
                <span className="text-xs font-bold w-[52px] text-right flex-shrink-0" style={{ color: "#374151" }}>
                  ${a.value}B
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="border border-gray-300 p-4 md:p-6">
          ...
          <div className="border-l-4 border-[#FCCC0A] bg-yellow-50 p-3 text-sm mb-6" style={{ color: "#374151" }}>
            96% of $14B in new city-funded spending went to filling gaps inherited from the Adams administration — not
            new programs.
          </div>
          ...
          {gapReduction.map((g) => (
            <div
              key={g.label}
              className="flex justify-between items-baseline py-2 border-b border-gray-200 last:border-b-0 gap-3"
            >
              <span className="text-sm flex-1" style={{ color: "#374151" }}>
                {g.label}
              </span>
              <span className={`text-sm font-bold`} style={{ color: g.pos ? "#00933C" : "#EE352E" }}>
                {g.value}
              </span>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="border border-gray-300 p-4 md:p-6">
          ...
          <div className="mb-5">
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-bold" style={{ color: "#EE352E" }}>
                0.57% actual
              </span>
            </div>
          ...
          <div className="mb-5">
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-bold" style={{ color: "#FF6319" }}>
                0.39% actual
              </span>
            </div>
            <div className="h-4 bg-gray-100 border-2 border-black overflow-hidden">
        ...
        <div className="border border-gray-300 p-4 md:p-6">
            ...
            <Table>
              ...
              <TableBody>
                {outyearData.map((row) => (
                  <TableRow
                    key={row.item}
                    className={row.isGap ? "font-bold border-t-2 border-[#0C2788] bg-gray-100" : ""}
                  >
                    <TableCell className="text-sm font-medium text-left">{row.item}</TableCell>
                    <TableCell
                      className={`text-sm text-right ${row.isGap && row.fy26.startsWith("–") ? "font-bold" : ""}`}
                      style={row.isGap && row.fy26.startsWith("–") ? { color: "#EE352E" } : {}}
                    >
                      {row.fy26}
                    </TableCell>
                    <TableCell
                      className={`text-sm text-right ${row.isGap && row.fy27.startsWith("–") ? "font-bold" : ""}`}
                      style={row.isGap && row.fy27.startsWith("–") ? { color: "#EE352E" } : {}}
                    >
                      {row.fy27}
                    </TableCell>
                    <TableCell
                      className={`text-sm text-right ${row.isGap && row.fy28.startsWith("–") ? "font-bold" : ""}`}
                      style={row.isGap && row.fy28.startsWith("–") ? { color: "#EE352E" } : {}}
                    >
                      {row.fy28}
                    </TableCell>
                    <TableCell
                      className={`text-sm text-right ${row.isGap && row.fy29.startsWith("–") ? "font-bold" : ""}`}
                      style={row.isGap && row.fy29.startsWith("–") ? { color: "#EE352E" } : {}}
                    >
                      {row.fy29}
                    </TableCell>
                    <TableCell
                      className={`text-sm text-right ${row.isGap && row.fy30.startsWith("–") ? "font-bold" : ""}`}
                      style={row.isGap && row.fy30.startsWith("–") ? { color: "#EE352E" } : {}}
                    >
                      {row.fy30}
                    </TableCell>
                  </TableRow>
          ...
          <div className="border-l-4 border-[#0C2788] bg-blue-50 p-3 mt-4 text-sm" style={{ color: "#374151" }}>
            FY2028–2030 structural gaps of $6.7B–$7.1B remain unresolved. Closing them requires either new revenue
            authority (Mamdani's Path 1 — taxing high earners and corporations) or further property tax hikes and
            reserve drawdowns (Path 2).
          </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="border-l-4 border-[#00933C] bg-green-50 p-3">
                <div className="flex justify-between items-baseline mb-1">
                  ...
                  <span className="text-xs font-bold uppercase" style={{ color: "#00933C" }}>
                    Tax authority
                  </span>
                </div>
                <p className="text-sm" style={{ color: "#374151" }}>
                  Income tax on earners &gt;$1M · corporate tax · fix NYC/NYS fiscal imbalance (NYC sends 54.5% of state
                  revenue, gets 40.5% back)
                </p>
              </div>
              <div className="border-l-4 border-[#EE352E] bg-red-50 p-3">
                <div className="flex justify-between items-baseline mb-1">
                  <span className="text-sm font-bold">Path 2 (Last resort)</span>
                  <span className="text-xs font-bold uppercase" style={{ color: "#EE352E" }}>
                    Austerity tools
                  </span>
                </div>
                <p className="text-sm" style={{ color: "#374151" }}>
                  Further property tax hikes · Rainy Day Fund · Retiree Health Trust drawdowns · affects 3M+ residential
                  units
                </p>
              </div>
            </div>
