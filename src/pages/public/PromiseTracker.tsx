...
export function PromiseTracker() {
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 sm:px-5 lg:px-6 py-5">
      <SEO 
        title="Promise Tracker - Mamdani Tracker | NYC Mayor Campaign Promises"
        description="Track every campaign promise made by NYC Mayor Zohran Mamdani. Filter by status (in progress, completed, stalled) and category (housing, transportation, education, healthcare, and more). Every commitment tracked with verified sources."
        keywords="Zohran Mamdani promises, NYC mayor promises, campaign promises tracker, NYC housing policy, NYC transportation, mayoral accountability, political promise tracking"
      />
      <div className="mb-5">
        <div className="border-t-4 border-[#0C2788] pt-4 mb-3">
          <h1 className="font-bold text-black tracking-tight" style={{ fontSize: '40px' }}>Promise Tracker</h1>
        </div>
      ...
      <div className="bg-white p-4 mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {statuses.map(status => (
                <button
                  ...
                  className={`px-3 py-2 text-xs font-bold transition-all border-0 cursor-pointer ${
                    selectedStatus === status
                      ? "bg-[#0C2788] text-white"
                      : "bg-gray-100 hover:bg-[#E9EDFB]"
                  }`}
                  style={selectedStatus !== status ? { color: '#374151' } : {}}
                >
                  {status}
                </button>
              ))}
        ...
        <div className="mt-3 pt-3 border-t">
          <p className="text-xs font-medium mb-0" style={{ color: '#374151' }}>
      ...
      {filteredPromises.length === 0 && (
        <div className="text-center py-5 bg-white border-4 border-[#0C2788]">
          <p className="text-lg mb-3" style={{ color: '#374151' }}>No promises match your filters.</p>
          <button
            onClick={() => {
